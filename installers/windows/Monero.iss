; Ditcoin Cosinus GUI Wallet Installer for Windows
; Copyright (c) 2017-2018, The Ditcoin Project
; Copyright (c) 2014-2017, The Monero Project
; See LICENSE

[Setup]
AppName=Ditcoin GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.15.0
DefaultDirName={pf}\Ditcoin GUI Wallet
DefaultGroupName=Ditcoin GUI Wallet
UninstallDisplayIcon={app}\ditcoin-wallet-gui.exe
...
; Ditcoin GUI wallet
Source: "bin\ditcoin-wallet-gui.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ditcoin GUI wallet log file
...
Source: "ditcoin-wallet-gui.log"; DestDir: "{app}"; Flags: comparetimestamp; Permissions: users-modify

; Ditcoin CLI wallet
Source: "bin\ditcoin-wallet-cli.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ditcoin wallet RPC interface implementation
Source: "bin\ditcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ditcoin daemon
Source: "bin\ditcoind.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Ditcoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "monero-daemon.bat"; DestDir: "{app}"; Flags: comparetimestamp;

; Ditcoin blockchain utilities
Source: "bin\ditcoin-blockchain-export.exe"; DestDir: "{app}"; Flags: comparetimestamp
Source: "bin\ditcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: comparetimestamp

; was present in 0.13.x, not present anymore in 0.15.0
; Source: "bin\ditcoin-utils-deserialize.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
; Filename: "{app}\ditcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\ditcoin');
  s := 'The default folder to store the Ditcoin blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\ditcoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    ...
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Ditcoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\ditcoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\ditcoin-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Ditcoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\ditcoin-wallet-gui.exe"
...
Name: "{group}\Utilities\Ditcoin Daemon"; Filename: "{app}\ditcoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\ditcoin-wallet-cli.exe"; WorkingDir: "{userdocs}\Ditcoin\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Ditcoin\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\ditcoin-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\ditcoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM ditcoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\ditcoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\ditcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\ditcoin-project\ditcoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\ditcoin-project\ditcoin-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
