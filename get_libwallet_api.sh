#!/bin/bash
MONERO_URL=https://github.com/ditcoin/ditcoin-core.git
...
MONERO_DIR=$ROOT_DIR/ditcoin
BUILD_LIBWALLET=false

# init and update ditcoin submodule
...
get_tag
# create local ditcoin branch
git -C $MONERO_DIR checkout -B $VERSIONTAG

# Merge ditcoin PR dependencies
...
git -C $MONERO_DIR config user.name "Ditcoin GUI"
git -C $MONERO_DIR config user.email "gui@ditcoin.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging ditcoin push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge ditcoin PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "ditcoin/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove ditcoin/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "cleaning up existing ditcoin build dir, libs and includes"
