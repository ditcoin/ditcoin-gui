...
# Base environnement
        ...
        docker build -f android32.Dockerfile -t ditcoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t ditcoin-gui-android .
        docker create -it --name ditcoin-gui-android ditcoin-gui-android bash

# Get the apk

        docker cp ditcoin-gui-android:/opt/android/ditcoin-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/ditcoin-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
